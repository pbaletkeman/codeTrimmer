...
**Code Trimmer** - A bash-based file cleanup utility for maintaining consistent whitespace and formatting standards across codebases.
...
- **As a Dev**, I want the script to recursively process subdirectories so that I can clean an entire project tree at once.
...
- **As a Dev**, I want to preview what changes would be made without modifying files so that I can verify the script will do what I expect.

**US-10: Binary File Skipping**

- **As a Dev**, I want the script to automatically skip binary files so that I don't corrupt non-text files.

### Performance & Safety Limits

**US-11: File Size Limit**

- **As a Ops**, I want to skip files larger than a configurable size (default 5MB) so that the script doesn't hang on huge files.

**US-12: File Count Limit**

- **As a Dev**, I want to process at most a configurable number of files (default 50) so that the script completes in a reasonable time.
...
- **FR-1:** The script MUST process all files in the specified directory matching the filter criteria.
- **FR-2:** The script MUST support recursive directory traversal into all subdirectories.
- **FR-3:** The script MUST trim all trailing whitespace from every line in processed files.
- **FR-4:** The script MUST ensure every file ends with exactly one newline character (empty line).
- **FR-5:** The script MUST reduce any sequence of more than 2 consecutive blank lines to exactly 2 blank lines.
- **FR-6:** The script MUST preserve the original file permissions after modification.
- **FR-7:** The script MUST preserve the original file timestamps where possible.

#### File Filtering

- **FR-8:** The script MUST support an inclusion list of file extensions (e.g., `--include "js,py,md"`).
- **FR-9:** The script MUST support an exclusion list of file extensions (e.g., `--exclude "min.js,lock"`).
- **FR-10:** The script MUST process files with no extension when `*` or empty string is in the inclusion list.
- **FR-11:** When both inclusion and exclusion lists are provided, exclusion takes precedence.
- **FR-12:** The script MUST automatically skip binary files (detected by file content analysis).
- **FR-13:** The script MUST skip hidden files and directories (starting with `.`) by default.
- **FR-14:** The script MUST provide an option to include hidden files (`--include-hidden`).

#### Safety Features

- **FR-15:** The script MUST create a backup of each file before modification with a `.bak` extension.
- **FR-16:** The script MUST support a dry-run mode (`--dry-run`) that reports changes without modifying files.
- **FR-17:** The script MUST restore from backup if an error occurs during file processing.
- **FR-18:** The script MUST skip files without read permissions and log a warning.
- **FR-19:** The script MUST skip files without write permissions and log a warning.
- **FR-20:** The script MUST skip symbolic links by default.
- **FR-21:** The script MUST provide an option to follow symbolic links (`--follow-symlinks`).

#### Performance Limits

- **FR-22:** The script MUST skip files larger than a configurable size limit (default: 5MB).
- **FR-23:** The script MUST stop processing after a configurable file count limit (default: 50 files).
- **FR-24:** The script MUST provide options to configure both limits: `--max-size <bytes>` and `--max-files <count>`.
- **FR-25:** The script MUST provide an option to disable limits: `--no-limits`.

#### Reporting & Output

- **FR-26:** The script MUST display a summary report showing:
  - Total files scanned
  ...
  - Execution time
- **FR-27:** The script MUST list all modified files by default.
- **FR-28:** The script MUST support a verbose mode (`--verbose` or `-v`) showing detailed processing for each file.
- **FR-29:** The script MUST support a quiet mode (`--quiet` or `-q`) showing only errors and final summary.
- **FR-30:** The script MUST use color-coded output (green for success, yellow for warnings, red for errors) when outputting to a terminal.
- **FR-31:** The script MUST disable color output when piped or redirected, or when `--no-color` is specified.

#### Error Handling

- **FR-32:** The script MUST return exit code 0 on successful completion.
- **FR-33:** The script MUST return exit code 1 for invalid arguments or configuration errors.
- **FR-34:** The script MUST return exit code 2 for file processing errors.
- **FR-35:** The script MUST log all errors to stderr.
- **FR-36:** The script MUST continue processing remaining files after a single file error.
- **FR-37:** The script MUST provide an option to stop on first error (`--fail-fast`).

#### Configuration

- **FR-38:** The script MUST accept all configuration via command-line arguments.
- **FR-39:** The script MUST support a configuration file (`--config <file>`) for persistent settings.
- **FR-40:** The script MUST support environment variables for common settings (e.g., `CODE_TRIMMER_MAX_SIZE`).
...
- **FR-43:** The script MUST provide a help message (`--help` or `-h`) documenting all options.
- **FR-44:** The script MUST provide a version flag (`--version`) showing the script version.
- **FR-45:** The script MUST provide usage examples in the help output.

### Non-Functional Requirements

#### Performance

- **NFR-1:** The script MUST process at least 100 small files (<10KB) per second on modern hardware.
- **NFR-2:** The script MUST not consume more than 100MB of memory during normal operation.
- **NFR-3:** The script MUST be interruptible (Ctrl+C) and clean up partial backups.

#### Compatibility

- **NFR-4:** The script MUST be compatible with Bash 4.0 or later.
- **NFR-5:** The script MUST run on Linux, macOS, and Windows (via Git Bash, WSL, or Cygwin).
- **NFR-6:** The script MUST use only POSIX-compliant utilities where possible.
- **NFR-7:** The script MUST document any non-POSIX dependencies (e.g., GNU-specific flags).

#### Security

- **NFR-8:** The script MUST NOT follow symbolic links outside the target directory tree by default.
- **NFR-9:** The script MUST validate all user inputs to prevent command injection.
- **NFR-10:** The script MUST handle filenames with special characters (spaces, quotes, newlines) safely.

#### Maintainability

- **NFR-11:** The script MUST include inline comments explaining complex logic.
- **NFR-12:** The script MUST follow consistent coding style (e.g., Google Shell Style Guide).
- **NFR-13:** The script MUST be modular with clearly defined functions.

#### Usability

- **NFR-14:** The script MUST provide clear, actionable error messages.
- **NFR-15:** The script MUST validate required arguments and show usage on error.
- **NFR-16:** The script MUST provide progress indication for long-running operations.

#### Reliability

- **NFR-17:** The script MUST be idempotent (running multiple times produces the same result).
- **NFR-18:** The script MUST handle unexpected termination gracefully (no corrupted files).
- **NFR-19:** The script MUST restore original files from backup on any processing error.
...
**When** I run `code-trimmer.sh /path/to/dir`
...
**When** I run `code-trimmer.sh --include "js,py" /path/to/dir`
...
**When** I run `code-trimmer.sh --include "js" --exclude "min.js" /path/to/dir`
...
**When** the script runs in normal mode
...
**When** I run `code-trimmer.sh --dry-run /path/to/dir`
...
**When** I run `code-trimmer.sh --max-size 20000000 /path/to/dir`
...
**When** I run `code-trimmer.sh --max-files 100 /path/to/dir`
...
**Given** the script runs in a terminal
...
**Given** the script is processing files
**When** I press Ctrl+C
**Then** the script should stop gracefully
